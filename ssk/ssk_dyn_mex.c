...
int nrhs, const mxArray *prhs[]) {
  ...
  // mexPrintf("Calling ssk_dyn_mex from inside\n");
  ...
  // double* array1 = (double *)mxMalloc(sizeof(double) * len1);
  // double* array2 = (double *)mxMalloc(sizeof(double) * len2);

  // mexPrintf("Getting data\n");


  double* array1 = (double *)mxGetData(prhs[0]);
  double* array2 = (double *)mxGetData(prhs[2]);


  // mexPrintf("Finished getting data\n");
  ...
  // mexPrintf("Actual call to ssk_dyn\n");
  // mexPrintf("array1[0]: %g, array[end]: %g, len1: %d, array2[0]: %g, array2[end]: %g, len2: %d, sublen: %d, lambda: %g\n", *array1, array1[len1-1], len1, *array2, array2[len2-1], len2, sublen, lambda);
  
  double retval;  
  retval = ssk_dyn(array1, len1, array2, len2, sublen, lambda);
  
  // mexPrintf("Retval: %g\n", retval);
  *b = retval;   

  // mexPrintf("freeing at end\n");
  // free heap space
  // mxFree(array1);
  // mxFree(array2);
  ...
  for(i = 1; i < sublen + 1; i++) {
    ...
    for(s_ind = 1; s_ind < len1 + 1; s_ind++) {
      ...
      for(t_ind = 1; t_ind < len2 + 1; t_ind++) {
        ...
        // 
        // IMPORTANT:
        // should't this make the complexity proportional to the second 
        // length squared? 
        for(x = 0; x < t_ind; x++) {
